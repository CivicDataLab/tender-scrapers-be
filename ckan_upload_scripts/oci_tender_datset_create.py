...
def upload_dataset(pkg_dict):
    ...
    try:
        package = ckan.action.package_create(name=pkg_name, title=pkg_dict['tender/title'],
                    owner_org=org_name, type='tender_dataset', fiscal_year=str(pkg_dict['Fiscal Year']), 
...
def main(): 
    ...
    raw_data = pd.read_csv(r'D:\CDL\ckan-api\ckan-api\data_to_upload_latest.csv', encoding='utf-8') 
